...
- **Autenticação**: usar o `better-auth` já existente, habilitando o plugin `emailAndPassword` (hoje só há Google + anônimo) e o plugin `admin` do better-auth para permitir que o backend crie contas de aluno em nome do PT com senha pré-definida, sem o fluxo normal de self-signup.
